...
function SinglePost(props) {
  ...
  const handleWindowScroll = useCallback(() => {
    ...
    window.scrollY > bind.ref.current.scrollHeight - 50 ? setIsBottom(true) : setIsBottom(false);
    ...
    <StyleSinglePost {...bind}>
        ...
        <StyledSinglePostUser>
            <StyledAvatar tabIndex="-1" size={52}>
                <Link href={`/user?userId=${!isEmpty(post) && post._embedded["author"]["0"].id}`}>
                    <a>
                        <img
                            src={
                                !isEmpty(post) && !post._embedded["author"]["0"].acf.avatar
                                    ? "/static/icons/default_avatar.png"
                                    : !isEmpty(post) ? post._embedded["author"]["0"].acf.avatar : "/static/icons/default_avatar.png"
                            }
                            alt={!isEmpty(post) ? post._embedded["author"]["0"].name : 'Alt Image'}
                        />
                    </a>
                </Link>
            </StyledAvatar>
        ...
        <StyledSinglePostImage
            ...
            alt={!isEmpty(post) && post._embedded["wp:featuredmedia"] ? !isEmpty(post) && post._embedded["wp:featuredmedia"]["0"].title.rendered : 'Default Post Image'}
        />
        <StyledSinglePostContent
            dangerouslySetInnerHTML={{ __html: !isEmpty(post) && post.content.rendered }}
        />
    </StyleSinglePost>
  );
}
